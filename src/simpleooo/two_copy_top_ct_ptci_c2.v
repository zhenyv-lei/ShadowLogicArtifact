...
//   wdata → diamond{rdata}         (NOT gnt!)
//
// PTCI controls both rdata AND gnt for copy2.
// Shared memd (one copy), rdata difference controlled by PTCI MUX.
// Expected: FAIL
// =============================================================================
...
// Shared rdata: both copies read from the same memd
// (use copy1's addr for shared read; when addrs are same this is correct)
wire [`REG_LEN-1:0] rdata_shared = memd[copy1.dmem_req_addr];

// Unconstrained alternatives (formal tool explores all values)
wire [`REG_LEN-1:0] rdata_unc;   // unconstrained rdata for copy2
wire                 delay_unc;   // unconstrained delay for copy2

// =========================================================================
// PTCI: Complete C2 Contract - Sticky-one logic
// =========================================================================

// Detect CPU output differences between two copies
...
// C2 contract mapping:
//   req  → {gnt, rdata}:  allow_gnt_diff includes sticky_req
//   addr → {gnt, rdata}:  allow_gnt_diff includes sticky_addr
//   wdata → {rdata} only: NOT included in allow_gnt_diff
// (SimpleOoO has no store/wdata, so wdata tracking is N/A)
wire allow_gnt_diff   = sticky_req | sticky_addr;
wire allow_rdata_diff = sticky_req | sticky_addr;

// =========================================================================
// PTCI MUX: control copy2's platform inputs
// =========================================================================
// copy1: always gets shared (baseline)
wire [`REG_LEN-1:0] rdata_1  = rdata_shared;
wire                 delayed_1 = 1'b0;

// copy2: PTCI-controlled
