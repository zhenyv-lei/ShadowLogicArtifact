...
`endif
  ...
  // cached_addr removed: timing now controlled by external dmem_resp_delayed
...
`endif
      ...
      // STEP.4: execute
      // Load timing controlled by external dmem_resp_delayed signal (PTCI)
      for (i=0; i<`ROB_SIZE; i=i+1) begin
        if (ROB_state[i]==`READY && ROB_exe_this_cycle[i]) begin
          ...
          // dmem_resp_delayed: controlled by PTCI (0=immediate, 1=delayed)
          if (ROB_op[i]==`INST_OP_LD && dmem_resp_delayed)
            ROB_state [i] <= `EXECUTING_1;
          else
            ROB_state [i] <= `FINISHED;
        end

        // Delayed load: EXECUTING_1 → EXECUTING_0 → FINISHED (3 cycles total)
...
`endif
  ...
  assign dmem_req_valid = |(ROB_exe_this_cycle & ROB_mem_rdwt & ROB_mem_valid);
  assign dmem_req_addr  = mem_read_addr;
