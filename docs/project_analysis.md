...
本文档中片上系统时序合约的命名遵循以下规则：

| 文档命名 | 代码命名 | 对应平台类型 | 说明 |
|---------|---------|------------|------|
| **C1-内存平台合约** | C1 | 理想内存（固定延迟） | 最强合约 |
| **C2-缓存平台合约** | C2 | 有缓存（地址依赖延迟） | 中等合约 |
| **C3-PMP平台合约** | C4（代码遗留） | 有中断控制器（PMP 保护） | 最弱合约 |

命名替换原则：
- 原设计中 C3 合约（无条件中断）因实验不可行已省略，原 C4 合约递补为 C3
- C1→C2→C3 保持升序递进关系（从强到弱），与格理论的偏序结构一致
- 中文后缀直接表达合约对应的平台类型
- 代码中的宏定义和文件名仍保留原始编号（C1/C2/C4），文档中统一使用新命名
...
| **CPU_C3** | CPU 在 C3-PMP 平台假设下是否安全 | CPU miter + C3 PTCI（+sticky_wdata_periph → allow_gnt_diff, allow_int_diff） |
| **Platform_C1** | 平台是否满足 C1-内存平台合约 | Platform miter，检查 addr→gnt 不存在 |
| **Platform_C2** | 平台是否满足 C2-缓存平台合约 | Platform miter，检查 wdata→gnt 不存在 |
| **Platform_C3** | 平台是否满足 C3-PMP平台合约 | Platform miter，检查 wdata→int 在 periph 外不存在 |
...
| E9 | Sodor | — (顺序) | **CPU_C3** (无PMP) | **CEX** (7 cycles) | 0.44s |
| — | Sodor | PMP | **CPU_C3** (有PMP) | **Proven** | 5.08s |
...
| — | Sodor-S | 中断控制器 | **Proven** | 1.70s | C3-PMP 修复后组合验证 |
...
**C2/C3-PMP 层级（Sodor + 中断控制器）：**

不兼容示例：
```
CPU 侧:
  Sodor CPU_C3 (无PMP) → FAIL    "Sodor 不满足 C3-PMP"
  Sodor CPU_C2 → PASS             "Sodor 满足 C2"

平台侧:
  中断控制器 Platform_C2 → FAIL   "中断控制器不满足 C2"
  中断控制器 Platform_C3 → PASS   "中断控制器满足 C3-PMP"

匹配分析:
  Sodor 需要 C2 平台，中断控制器只满足 C3-PMP → 不兼容！
```

兼容示例（修复 CPU 侧）：
```
CPU 侧:
  Sodor-S CPU_C3 (PMP) → PASS     "Sodor-S 满足 C3-PMP"
...
**C3-PMP 修复（Sodor + 中断控制器）：**

修复 CPU 侧（增加 PMP 约束）
```
Sodor-S CPU_C3 (PMP) → PASS     "PMP 禁止秘密写入外设地址"
中断控制器 Platform_C3 → PASS    "满足 C3-PMP"
